                                ...
                                <div class="col-md-12">
									...
									<div class="table-responsive m-t-40">
										<table id="myTable" class="table table-bordered table-striped">
											...
											<tbody>
												<?php $i=1; foreach($service->result() as $row){?>
												<tr id="old<?php echo $row->ser_id;?>">
													<td><?php echo $i;?></td>													
													<td><img src="<?php echo base_url();?>assets/img/<?php echo $row->ser_img;?>" width="100%"></td>									
													<td><?php echo $row->ser_title;?></td>									
													<td><?php echo $row->ser_desc;?></td>
													<td>
														<span id="view<?php echo $row->ser_id;?>"></span>
														<span id="active<?php echo $row->ser_id;?>">
															<?php if($row->ser_status == 1){ $status = "Active" ; }else{ $status = "Inactive" ;}?>
															<?php if($row->ser_status == 1){?>
																<button class="btn btn-info shi" onclick="inactive(<?php echo $row->ser_id;?>)" ><?php echo $status;?></button>
															<?php }else{?>
																<button class="btn btn-warning shi" onclick="active(<?php echo $row->ser_id;?>)"><?php echo $status;?></button>
															<?php } ?>
														</span>
													</td>													
													<td>													
														<a class="btn btn-info shi" href="<?php echo base_url();?>admin/edit_services/<?php echo $row->ser_id;?>">Edit</a>
													</td>
													<td>
														<button class="btn btn-danger shi" onclick="deletecatogry(<?php echo $row->ser_status;?>)">Delete</button>
													</td>
												</tr>		
												<?php $i++; } ?>
											</tbody>
				...
				<div class="modal-content">
					<div class="modal-body">
					    <form action="<?php echo base_url();?>admin/add_homebanner" method="POST" enctype="multipart/form-data">
					    	 ...
					    	 <label>Title</label>
					    	 <input type="text" class="form-control" name="title"  value="" placeholder="Title" required><br>
					    	 <label>Image</label>
            ...
            function deletecatogry(id){
				...
				if(a == true){
					$.ajax({
						type: "POST",
						data: {'id':id},
						url : "<?php echo base_url();?>admin/delete_servics",
						dataType:"json",
		...
		<script>
			function inactive(id){
				$.ajax({
					type: "POST",
					data: {'id':id},
					url : "<?php echo base_url();?>admin/inactive_servics",
					dataType:"json",
					success: function(data){
						var viewhtml ='';
						if(data.status > 0){
		...
		<script>
			function active(id){
				$.ajax({
					type: "POST",
					data: {'id':id},
					url : "<?php echo base_url();?>admin/active_servics",
					dataType:"json",
					success: function(data){
						var viewhtml ='';
						if(data.status > 0){
