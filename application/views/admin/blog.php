                ...
                <div class="row page-titles">
                    ...
                    <div class="col-md-7 align-self-center text-right">
                        <div class="d-flex justify-content-end align-items-center">
                            ...
                            <a  class="btn btn-info d-none d-lg-block m-l-15" href="<?php echo base_url();?>admin/add_blog"> Add Blog </a>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="col-md-12">
									...
									<div class="table-responsive m-t-40">
										<table id="myTable" class="table table-bordered table-striped">
											...
											<tbody>
												<?php $i=1; foreach($blog->result() as $row){?>
												<tr id="old<?php echo $row->blog_id;?>">
													<td><?php echo $i;?></td>													
													<td><img src="<?php echo base_url();?>assets/img/<?php echo $row->blog_img;?>" width="100%"></td>
													<td><?php echo $row->blog_title;?></td>
													<td><?php echo $row->blog_desc;?></td>
													<td>
														<span id="view<?php echo $row->blog_id;?>"></span>
														<span id="active<?php echo $row->blog_id;?>">
															<?php if($row->blog_status == 1){ $status = "Active" ; }else{ $status = "Inactive" ;}?>
															<?php if($row->blog_status == 1){?>
																<button class="btn btn-info shi" onclick="inactive(<?php echo $row->blog_id;?>)" ><?php echo $status;?></button>
															<?php }else{?>
																<button class="btn btn-warning shi" onclick="active(<?php echo $row->blog_id;?>)"><?php echo $status;?></button>
															<?php } ?>
														</span>
													</td>													
													<td>													
														<a class="btn btn-warning shi" href="<?php echo base_url();?>admin/edit_blog/<?php echo $row->blog_id;?>">Edit</a>
													</td>
													<td>
														<button class="btn btn-danger shi" onclick="deletecatogry(<?php echo $row->blog_id;?>)">Delete</button>
													</td>
												</tr>		
												<?php $i++; } ?>
											</tbody>
